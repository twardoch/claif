...
from .client import query, query_random, query_all, _client
...
class ClaifCLI:
        ...
        try:
            ...
            if compare:
                ...
                for provider, messages in results.items():
                    content = "\n".join(
                        format_response(msg) for msg in messages
                    ) if messages else "[dim]No response[/dim]"
    ...
    def server(
        ...
        """
        console.print(f"[bold]Starting CLAIF MCP Server[/bold]")
